...
<body>
	...
	<form action="<%=ORSView.TIMETABLE_LIST_CTL%>" method="post">
		...
		<center>
			...
			<table width="100%">
				<tr>
					<td align="center"><label>Course Name :</label> <%=HTMLUtility.getList("clist", String.valueOf(bean.getId()), clist)%>
						<label>Subject Name :</label> <%=HTMLUtility.getList("slist", String.valueOf(bean.getId()), sList)%>
						<label>Exam Date :</label> <input type="text" readonly="readonly"
						id="udat" size="26" placeholder="Select Date" name="ExDate"
						value="<%=ServletUtility.getParameter("ExDate", request)%>">
						<%-- <label>Semester :</label> <input type="text" size="26"
						placeholder="Select semester" name="Semester"
